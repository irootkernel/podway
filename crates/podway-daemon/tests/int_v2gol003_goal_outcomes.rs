...
fn v2gol003_general_decision_never_creates_goal_assessment_state() {
    ...
    let selector = runtime::selector(workspace.main());
    let manager = Arc::new(runtime::manager(workspace.temporary_path()));
    let dispatcher = runtime::dispatcher(manager, "v2gol003-general");
    ...
    let before = runtime::status(&dispatcher, &selector, 103_203, session_id);
    ...
    let unchanged = runtime::status(&dispatcher, &selector, 103_205, session_id);
