...
#[test]
fn context_rebind_preserves_the_same_durable_key_and_generation() {
    let registry = WorkspaceSchedulerRegistryV1::new();
    let durable_key = key(3);
    let scheduler = registry
        .get_or_create(durable_key.clone(), || 11_usize)
        .expect("initial scheduler generation is valid");
    let original_context = scheduler.context_snapshot();

    let prior_context = scheduler.rebind_context_serialized(Arc::new(22));
    let same_scheduler = registry
        .get_or_create(durable_key.clone(), || 33_usize)
        .expect("active scheduler is returned without running its factory");

    assert_eq!(*original_context, 11);
    assert_eq!(*prior_context, 11);
    assert_eq!(*scheduler.context_snapshot(), 22);
    assert!(Arc::ptr_eq(&scheduler, &same_scheduler));
    assert_eq!(scheduler.key(), &durable_key);
    assert_eq!(scheduler.generation(), same_scheduler.generation());
}

#[test]
fn context_rebind_waits_for_in_flight_serialized_operation() {
    let registry = WorkspaceSchedulerRegistryV1::new();
    let scheduler = registry
        .get_or_create(key(13), || 11_usize)
        .expect("initial scheduler generation is valid");
    ...
    entered.wait();
    let (rebound_sender, rebound_receiver) = mpsc::channel();
    let rebind = {
        let scheduler = Arc::clone(&scheduler);
        thread::spawn(move || {
            let prior = scheduler.rebind_context_serialized(Arc::new(22));
            rebound_sender
                .send(*prior)
                .expect("main test thread receives the prior context");
        })
    };
    assert!(
        rebound_receiver
            ...
            .is_err(),
        "rebind must wait while an earlier serialized operation is in flight"
    );

    release.wait();
    holder.join().expect("serialized holder does not panic");
    assert_eq!(
        rebound_receiver
            .recv_timeout(Duration::from_secs(5))
            .expect("rebind completes after the in-flight operation"),
        11
    );
    rebind.join().expect("rebind thread does not panic");
    assert_eq!(*scheduler.context_snapshot(), 22);
}
