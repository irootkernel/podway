...
pub fn graph_prepared_session_state_from_procedure_v2_snapshot(
    snapshot: ProcedureSnapshotV2,
    task_title: &str,
    session_id: SessionId,
    created_at: UnixMillis,
...
fn graph_start_current_task_json_v2(expected: &GraphStartCurrentTaskV2) -> Value {
    match expected {
...
fn decode_procedure_v2_start_execution_v1(
    ...
    let expected_current = match value_v1(object, "expected_current")? {
        ...
        Value::Object(expected) => {
            let session_id = value_typed_v1(expected, "session_id")?;
            let session_revision = value_typed_v1(expected, "session_revision")?;
            match expected.get("mode").and_then(Value::as_str) {
                None => {
                ...
                Some("eligible") => {
                    require_exact_keys_v1(expected, &["mode", "session_id", "session_revision"])?;
                    GraphStartCurrentTaskV2::Eligible {
                        session_id,
                        session_revision,
                    }
    ...
    let created_at = UnixMillis::new(value_u64_v1(
...
struct AdmittedProcedureV2MutationV1 {
    ...
    attached_artifacts: Vec<ResolvedBatchArtifactV1>,
}

#[derive(Clone, Debug)]
enum AdmittedLifecycleMutationKindV1 {
    Begin {
...
#[derive(Clone, Debug)]
struct AdmittedLifecycleMutationV1 {
    workspace_id: WorkspaceId,
    session_id: SessionId,
    session_revision: Revision,
    kind: AdmittedLifecycleMutationKindV1,
}

fn lifecycle_mutation_execution_document_v1(
    ...
    let (preconditions, payload) = match request.command() {
        ...
        _ => return Err(invalid_execution_v1("not a lifecycle mutation")),
    };
    let document = json!({
        "command": request.command().command_name(),
        "execution_version": EXECUTION_DOCUMENT_VERSION_V18,
    ...
    CanonicalExecutionJsonV1::new(
        canonicalize_json_v1(&document)
            .map_err(|_| invalid_execution_v1("lifecycle mutation cannot be canonicalized"))?,
    )
    .map_err(ExecutionErrorV1::InvalidStoreValue)
}

fn decode_lifecycle_mutation_execution_v1(
    source: &str,
) -> Result<AdmittedLifecycleMutationV1, ExecutionErrorV1> {
    let root: Value = serde_json::from_str(source)
        .map_err(|_| invalid_execution_v1("lifecycle mutation execution is not JSON"))?;
    let object = root
        .as_object()
        .ok_or_else(|| invalid_execution_v1("lifecycle mutation execution root is invalid"))?;
    require_exact_keys_v1(
        object,
        &[
            "command",
    ...
    if value_u64_v1(object, "execution_version")? != u64::from(EXECUTION_DOCUMENT_VERSION_V18) {
        return Err(invalid_execution_v1(
            "lifecycle mutation execution version is invalid",
        ));
    }
    let preconditions = value_object_v1(object, "preconditions")?;
    require_exact_keys_v1(preconditions, &["session_id", "session_revision"])?;
    let payload = value_object_v1(object, "payload")?;
    let kind = match value_string_v1(object, "command")? {
        "session.begin" => {
            require_exact_keys_v1(payload, &["initial_goal"])?;
            let initial_goal = match value_v1(payload, "initial_goal")? {
                Value::Null => None,
                Value::Object(goal) => {
                    require_exact_keys_v1(goal, &["actor", "criteria", "goal"])?;
                    let actor = optional_string_v1(goal, "actor")?
                        ...
                        .transpose()
                        .map_err(ExecutionErrorV1::BoundaryDomain)?;
                    Some(GraphInitialGoalV2::new(
                        GoalStatementV2::new(value_string_v1(goal, "goal")?.to_owned())
                            .map_err(ExecutionErrorV1::BoundaryDomain)?,
                        decode_goal_criteria_v2(value_v1(goal, "criteria")?)?,
                        actor,
                    ))
                }
                _ => return Err(invalid_execution_v1("initial goal is invalid")),
            };
            AdmittedLifecycleMutationKindV1::Begin {
                initial_goal,
                fresh_attempt_id: value_optional_typed_v1(object, "fresh_attempt_id")?
                    .ok_or_else(|| invalid_execution_v1("begin attempt identity is absent"))?,
            }
        }
        "session.terminal_disposition" => {
            require_exact_keys_v1(payload, &["actor", "disposition"])?;
            if !value_v1(object, "fresh_attempt_id")?.is_null() {
                return Err(invalid_execution_v1(
                    "disposition attempt identity is invalid",
                ));
            }
            let actor = optional_string_v1(payload, "actor")?
                .map(|value| ActorAttributionV2::new(value.to_owned()))
                .transpose()
                .map_err(ExecutionErrorV1::BoundaryDomain)?;
            let disposition = value_object_v1(payload, "disposition")?;
            let disposition = match value_string_v1(disposition, "kind")? {
                "handed_off" => {
                    require_exact_keys_v1(disposition, &["kind", "reference", "summary"])?;
                    TerminalDispositionInputV1::HandedOff {
                        summary: value_string_v1(disposition, "summary")?.to_owned(),
                        reference: value_string_v1(disposition, "reference")?.to_owned(),
                    }
                }
                "not_required" => {
                    require_exact_keys_v1(disposition, &["kind", "reason"])?;
                    TerminalDispositionInputV1::NotRequired {
                        reason: value_string_v1(disposition, "reason")?.to_owned(),
                    }
                }
                _ => return Err(invalid_execution_v1("terminal disposition kind is invalid")),
            };
            AdmittedLifecycleMutationKindV1::TerminalDisposition { disposition, actor }
        }
        _ => {
            return Err(invalid_execution_v1(
                "lifecycle mutation command is invalid",
            ));
        }
    };
    let _: WorktreeSelectorWireV1 =
        serde_json::from_value(value_v1(object, "selector")?.clone())
            .map_err(|_| invalid_execution_v1("lifecycle mutation selector is invalid"))?;
    Ok(AdmittedLifecycleMutationV1 {
        workspace_id: value_typed_v1(object, "workspace_id")?,
        session_id: value_typed_v1(preconditions, "session_id")?,
        session_revision: value_typed_v1(preconditions, "session_revision")?,
        kind,
    })
}

#[derive(Clone, Debug)]
struct ResolvedBatchArtifactV1 {
    item_id: ItemId,
...
fn is_procedure_v2_graph_mutation_v1(command: &SliceCommandV1) -> bool {
    is_procedure_v2_graph_mutation_v7(command)
        ...
        || matches!(command, SliceCommandV1::ItemRecordMany(_))
}
...
fn procedure_v2_mutation_execution_document_v1(
    ...
    let document = if matches!(admitted.command, SliceCommandV1::ItemRecordMany(_)) {
        json!({
            "attached_artifacts": admitted.attached_artifacts.iter().map(|resolved| json!({
                "artifact": artifact_value_v1(&resolved.artifact),
                "item_id": resolved.item_id,
            })).collect::<Vec<_>>(),
            "command": admitted.command.command_name(),
            "execution_version": EXECUTION_DOCUMENT_VERSION_V16,
            "payload": payload,
            "preconditions": preconditions,
            "selector": admitted.selector,
            "workspace_id": admitted.workspace_id,
        })
    } else if is_procedure_v2_graph_mutation_v8(&admitted.command) {
        json!({
...
fn decode_procedure_v2_mutation_execution_v1(
    ...
    } else if execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V16) {
        require_exact_keys_v1(
            object,
            &[
                "attached_artifacts",
    ...
    let command = decode_command_components_v1(
        if execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V16) {
            execution_version
        } else {
    ...
    if (execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V7)
        ...
        || (execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V8)
            && !is_procedure_v2_graph_mutation_v8(&command))
        || (execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V16)
            && !matches!(command, SliceCommandV1::ItemRecordMany(_)))
    {
        return Err(invalid_execution_v1(
    ...
    let fresh_attempt_id = if execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V16) {
        None
    } else {
        value_optional_typed_v1(object, "fresh_attempt_id")?
    };
    ...
    let attached_artifact = if execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V16) {
        None
    } else {
        match value_v1(object, "attached_artifact")? {
            Value::Null => None,
    ...
    let attached_artifacts = if execution_version == u64::from(EXECUTION_DOCUMENT_VERSION_V16) {
        decode_resolved_batch_artifacts_v1(value_v1(object, "attached_artifacts")?)?
    } else {
        Vec::new()
    };
    match &command {
...
fn prepare_procedure_v2_item_mutations_v1(
    admitted: &AdmittedProcedureV2MutationV1,
) -> Result<Vec<ActiveItemMutationRequestV2>, ExecutionErrorV1> {
    let SliceCommandV1::ItemRecordMany(input) = &admitted.command else {
        return Err(invalid_execution_v1(
            "Procedure v2 item batch preparation received another command",
        ));
    };
    let expected_artifact_items = input
        .operations
        .iter()
        .filter(|operation| {
            matches!(
        ...
        .map(|operation| operation.item_id.clone())
        .collect::<Vec<_>>();
    if expected_artifact_items.len() != admitted.attached_artifacts.len()
        || expected_artifact_items
            .iter()
            .zip(&admitted.attached_artifacts)
            .any(|(expected, resolved)| expected != &resolved.item_id)
    {
        return Err(invalid_execution_v1(
            "Procedure v2 item batch artifact resolutions do not match the command",
        ));
    }
    ...
    input
        ...
        .iter()
        .map(|operation| {
            let mutation = match &operation.disposition {
                ItemRecordManyDispositionV1::Clear { clear: true } => ActiveItemMutationV2::Clear,
                ItemRecordManyDispositionV1::Clear { clear: false } => {
                    return Err(invalid_execution_v1(
                        "Procedure v2 item batch clear disposition is invalid",
                    ));
                }
                ItemRecordManyDispositionV1::Record { record } => match record {
                    ItemRecordValueV1::Confirm { value: true } => ActiveItemMutationV2::Check,
                    ItemRecordValueV1::Confirm { value: false } => ActiveItemMutationV2::Uncheck,
                    ItemRecordValueV1::Text { value } | ItemRecordValueV1::Choice { value } => {
                        ActiveItemMutationV2::Set {
                            value: value.clone(),
                        }
                    }
                    ItemRecordValueV1::Integer { value } => ActiveItemMutationV2::Set {
                        value: value.to_string(),
                    },
                    ItemRecordValueV1::List { value } => ActiveItemMutationV2::ReplaceList {
                        values: value.clone(),
                    },
                            ...
                            admitted
                                ...
                                .ok_or_else(|| {
                                    invalid_execution_v1(
                                        "Procedure v2 item batch artifact resolution is absent",
                                    )
                                })?;
                        artifact_index += 1;
                        ActiveItemMutationV2::Attach {
            ...
            Ok(ActiveItemMutationRequestV2::new(
                ...
                operation.expected_item_revision,
                mutation,
            ))
        })
        .collect()
    ...
    Workspaces: WorkspaceRevalidatorV1,
{
    ...
    pub fn admit_procedure_v2_typed_start_for_workspace_with_response_context(
        ...
        let (start, expected_current, command) = match request.command() {
        ...
        let state = match &start.source {
            SessionStartSourceV1::Procedure { procedure } => {
                let snapshot = self
                    .procedures
                    .load_workspace_procedure_snapshot_v2(&binding, procedure, snapshot_id, now)
                    .map_err(ProcedureV2StartPreparationErrorV1::Source)?;
                let actual = snapshot.digest().clone();
                match start.expected_procedure_digest.as_ref() {
                    None => {
                        return Err(
                            ProcedureV2StartPreparationErrorV1::DigestConfirmationRequired {
                                procedure_digest: actual,
                            },
                        );
                    }
    ...
    pub fn admit_procedure_v2_mutation_for_workspace_with_response_context(
            ...
            if !matches!(
                ...
                Some(version)
                    if version == u64::from(EXECUTION_DOCUMENT_VERSION_V7)
                        ...
                        || version == u64::from(EXECUTION_DOCUMENT_VERSION_V16)
            ) {
                return Ok(None);
            }
        ...
        if let SliceCommandV1::ItemRecordMany(input) = request.command() {
            if input.preconditions.expected_session_revision != state.trace().revision() {
                return Err(StoreErrorV1::PreconditionConflictV1 {
                    expected: Some(input.preconditions.expected_session_revision),
                    actual: Some(state.trace().revision()),
                }
                .into());
            }
            ...
            if actual_attempt.as_ref() != Some(&input.preconditions.expected_attempt_id) {
                return Err(StoreErrorV1::ProcedureV2PreconditionFailedV1 {
                    failure: PersistedGraphMutationFailureV2::AttemptNotCurrent {
                        expected: input.preconditions.expected_attempt_id.clone(),
        ...
        let attached_artifacts = if let SliceCommandV1::ItemRecordMany(input) = request.command() {
            input
                .operations
                .iter()
                .filter_map(|operation| match &operation.disposition {
                    ItemRecordManyDispositionV1::Record { record } => record
                        .artifact_source()
                        .map(|source| (operation.item_id.clone(), source)),
                    ItemRecordManyDispositionV1::Clear { .. } => None,
                })
                .map(|(item_id, source)| {
                    let artifact = match source.map_err(|_| {
                        invalid_execution_v1("Procedure v2 item batch artifact source is invalid")
                    })? {
                        ItemAttachSourceV1::Path { path, media_type } => {
                            let artifact = self
                                .artifacts
                                ...
                                .map_err(BoundaryDispositionV1::from)
                                .map_err(ExecutionErrorV1::from_boundary)?;
                            validate_local_attached_artifact_v1(
                                &path,
                                media_type.as_deref(),
                                &artifact,
                            )
                        ...
                        ItemAttachSourceV1::OpaqueReference {
                            ...
                            reference, digest, size_bytes, media_type,
                        )
                        .map_err(ExecutionErrorV1::BoundaryDomain)?,
                    };
                    Ok(ResolvedBatchArtifactV1 { item_id, artifact })
                })
                .collect::<Result<Vec<_>, ExecutionErrorV1>>()?
        } else {
            Vec::new()
        ...
        if matches!(admitted.command, SliceCommandV1::ItemRecordMany(_)) {
            let prepared = prepare_procedure_v2_item_mutations_v1(&admitted)?;
            let input = match &admitted.command {
                SliceCommandV1::ItemRecordMany(input) => input,
                _ => unreachable!("batch admission was matched above"),
            };
            state
                ...
                .map_err(|error| {
                    PersistedGraphMutationFailureV2::try_from(&error)
                        .map(|failure| {
                            ExecutionErrorV1::Store(StoreErrorV1::ProcedureV2PreconditionFailedV1 {
                                failure,
                            })
                        })
                        .unwrap_or_else(|_| {
                            ExecutionErrorV1::BoundaryDomain(DomainError::InvalidState {
                                reason: "Procedure v2 item batch is invalid",
                            })
                        })
                })?;
        }
    ...
    pub fn admit_procedure_v2_typed_mutation_for_workspace_with_response_context(
        ...
        match request.command() {
            ...
            | ProcedureV2MutationCommandV1::SessionTerminalDisposition(_) => self
                .admit_lifecycle_mutation_for_workspace_with_response_context(
                    expected_workspace,
                    request,
                    idempotency_key,
                    response_context,
    ...
    fn admit_lifecycle_mutation_for_workspace_with_response_context(
        &self,
        expected_workspace: &WorkspaceBindingV1,
        request: &ProcedureV2MutationRequestV1,
        idempotency_key: IdempotencyKeyV1,
            ...
            .read_idempotent_execution(expected_workspace.identity(), &idempotency_key)?
        {
            ...
            let admitted = decode_lifecycle_mutation_execution_v1(canonical_execution.as_str())?;
            if admitted.workspace_id != *expected_workspace.identity().workspace_uuid() {
                return Err(invalid_execution_v1(
                    "lifecycle replay workspace identity is invalid",
                ));
            }
        ...
        if binding.identity() != expected_workspace.identity() {
            return Err(invalid_execution_v1(
                "lifecycle mutation workspace identity changed",
            ));
        }
        let (preconditions, command, fresh_attempt_id) = match request.command() {
            ProcedureV2MutationCommandV1::SessionBegin(input) => (
                &input.preconditions,
                DomainCommand::SessionBegin,
                Some(self.ids.next_attempt_id()),
            ),
            ProcedureV2MutationCommandV1::SessionTerminalDisposition(input) => (
                &input.preconditions,
                DomainCommand::SessionTerminalDisposition,
                None,
            ),
            _ => return Ok(None),
        };
        let canonical_execution = lifecycle_mutation_execution_document_v1(
            request,
            binding.identity().workspace_uuid(),
            fresh_attempt_id.as_ref(),
        )?;
        ...
        .with_session_identity(AdmissionSessionIdentityV1::Exact(
            preconditions.expected_session_id.clone(),
        ));
        let durable = match response_context {
            Some(context) => durable.with_response_context(context),
            None => durable,
    ...
    pub fn execute_next_with_graph_v2(
        ...
        let receipt = match version {
            ...
            version if version == u64::from(EXECUTION_DOCUMENT_VERSION_V18) => {
                let admitted = decode_lifecycle_mutation_execution_v1(
                    claimed.execution().canonical_execution().as_str(),
                )?;
                self.execute_lifecycle_mutation_claimed(&claimed, admitted, now)?
            }
            _ => {
                return Err(invalid_execution_v1(
                    "Procedure v2 execution version is unsupported",
    ...
    fn execute_lifecycle_mutation_claimed(
        &self,
        claimed: &ClaimedJobV1,
        admitted: AdmittedLifecycleMutationV1,
        now: UnixMillis,
    ) -> Result<TerminalReceiptV1, ExecutionErrorV1> {
        if admitted.workspace_id != *claimed.claim().identity().workspace_uuid() {
            return Err(invalid_execution_v1(
                "lifecycle execution workspace does not match claim",
            ));
        }
        let state = self
            .store
            ...
            .graph_state()
            .cloned()
            .ok_or_else(|| ExecutionErrorV1::SessionIdentityMismatch {
                expected: admitted.session_id.clone(),
                actual: None,
            })?;
        if state.trace().session_id() != &admitted.session_id {
            return Err(ExecutionErrorV1::SessionIdentityMismatch {
                expected: admitted.session_id,
                actual: Some(state.trace().session_id().clone()),
            });
        }
        ...
        match admitted.kind {
            AdmittedLifecycleMutationKindV1::Begin {
                ...
                let outcome = match state.begin_v2(
                    admitted.session_revision,
                ...
                let operation = PersistedGraphTerminalOperationV2::begin(
                    fresh_attempt_id,
                    outcome.goal_revision().map(GoalRevisionNumberV2::get),
                )
                .map_err(|_| invalid_execution_v1("begin operation cannot be persisted"))?;
                let result = DomainResult::SessionChanged {
                    session_id: state.trace().session_id().clone(),
            ...
            AdmittedLifecycleMutationKindV1::TerminalDisposition { disposition, actor } => {
                ...
                let operation = PersistedGraphTerminalOperationV2::terminal_disposition(
                    ...
                    stable_reference,
                    reason,
                    actor.map(|actor| actor.as_str().to_owned()),
                    now.get(),
                )
                .map_err(|_| invalid_execution_v1("terminal disposition cannot be persisted"))?;
                let result = DomainResult::SessionChanged {
                    session_id: state.trace().session_id().clone(),
                    revision_before: state.trace().revision(),
    ...
    fn execute_procedure_v2_mutation_claimed(
        ...
        match &admitted.command {
            ...
            SliceCommandV1::SessionReset(input) => {
                ...
                let (mode, progress_summary) = match &input.mode {
                    SessionDeletionModeV1::Eligible => (PersistedGraphResetModeV2::Eligible, None),
                    SessionDeletionModeV1::Force { progress_summary } => (
                        PersistedGraphResetModeV2::Force,
                        Some(progress_summary.clone()),
                    ),
                ...
                match self.store.commit_graph_smart_reset_terminal_v2(
                    ...
                    Err(StoreErrorV1::SessionResetNotEligibleV1 {
                        lifecycle,
            ...
            SliceCommandV1::ItemRecordMany(input) => {
                let prepared = prepare_procedure_v2_item_mutations_v1(&admitted)?;
                let active = state.trace().active_attempt().ok_or_else(|| {
                    invalid_execution_v1("Procedure v2 claimed item batch has no active attempt")
                })?;
                for resolved in &admitted.attached_artifacts {
                    if resolved.artifact.location_kind() != ArtifactLocationKindV1::LocalPath {
                        continue;
                    }
                    if let Err(error) = self.artifacts.revalidate_local_artifact(
                        workspace,
                        &resolved.item_id,
                        &resolved.artifact,
                    ) {
                ...
                let outcome = match state.mutate_active_items_v2(
                    &input.preconditions.expected_attempt_id,
                    &prepared,
                    now,
                ) {
                ...
                let items = input
                    ...
                    .zip(outcome.items())
                    .map(|(operation, item)| {
                        if operation.item_id != *item.item_id() {
                            return Err(invalid_execution_v1(
                                "Procedure v2 item batch outcome order is invalid",
                            ));
                        }
                        Ok(PersistedGraphItemMutationV2::new(
                            operation.item_id.clone(),
                            operation.expected_item_revision,
                            item.changed(),
                            item.item_revision(),
                ...
                .map_err(|_| {
                    invalid_execution_v1("Procedure v2 item batch operation cannot be persisted")
                })?;
                let result = DomainResult::ItemsChanged {
                    session_id: state.trace().session_id().clone(),
                    revision_before: state.trace().revision(),
                    revision_after: outcome.state().trace().revision(),
                    changed: outcome.changed(),
...
fn command_for_admission_v1(command: &SliceCommandV1) -> Result<DomainCommand, ExecutionErrorV1> {
    let command = match command {
        ...
        SliceCommandV1::ItemRecordMany(_) => DomainCommand::ItemRecordMany,
...
fn expected_session_id_v1(command: &SliceCommandV1) -> Option<&SessionId> {
    match command {
        ...
        SliceCommandV1::ItemRecordMany(input) => Some(&input.preconditions.expected_session_id),
        SliceCommandV1::WorkspaceInit(_)
        | SliceCommandV1::WorkspaceResetAll(_)
...
fn decode_resolved_batch_artifacts_v1(
    value: &Value,
) -> Result<Vec<ResolvedBatchArtifactV1>, ExecutionErrorV1> {
    let values = value.as_array().ok_or_else(|| {
        invalid_execution_v1("Procedure v2 batch artifact resolutions are invalid")
    })?;
    let mut resolved = values
        .iter()
        .map(|value| {
            let object = value.as_object().ok_or_else(|| {
                invalid_execution_v1("Procedure v2 batch artifact resolution is invalid")
            })?;
            require_exact_keys_v1(object, &["artifact", "item_id"])?;
            Ok(ResolvedBatchArtifactV1 {
                item_id: value_typed_v1(object, "item_id")?,
                artifact: decode_artifact_value_v1(value_object_v1(object, "artifact")?)?,
            })
        })
        .collect::<Result<Vec<_>, ExecutionErrorV1>>()?;
    resolved.sort_by(|left, right| left.item_id.as_str().cmp(right.item_id.as_str()));
    if resolved
        .windows(2)
        .any(|pair| pair[0].item_id == pair[1].item_id)
    {
        return Err(invalid_execution_v1(
            "Procedure v2 batch artifact resolution has duplicate item IDs",
        ));
    }
    Ok(resolved)
}
...
fn execution_components_v1(command: &SliceCommandV1) -> (Value, Value) {
    match command {
        ...
        SliceCommandV1::SessionStartReplace(input) => (
            ...
            json!({
                "deletion": session_deletion_mode_value_v1(&input.mode),
...
fn session_deletion_mode_value_v1(mode: &SessionDeletionModeV1) -> Value {
    match mode {
...
fn decode_session_deletion_mode_v1(
    payload: &Map<String, Value>,
) -> Result<SessionDeletionModeV1, ExecutionErrorV1> {
    if payload.get("confirmed").and_then(Value::as_bool) == Some(true) {
        require_exact_keys_v1(payload, &["confirmed"])?;
        return Ok(SessionDeletionModeV1::LegacyConfirmed);
    }
    match value_string_v1(payload, "mode")? {
        "eligible" => {
            require_exact_keys_v1(payload, &["mode"])?;
            Ok(SessionDeletionModeV1::Eligible)
        }
        "force" => {
...
fn decode_command_components_v1(
    ...
    match command {
        ...
        "session.start_replace" => {
            ...
            Ok(SliceCommandV1::SessionStartReplace(
                podway_protocol::SessionStartReplaceV1 {
                    ...
                    mode: SessionDeletionModeV1::LegacyConfirmed,
        ...
        "session.reset" => Ok(SliceCommandV1::SessionReset(SessionResetV1 {
            mode: decode_session_deletion_mode_v1(payload)?,
            dry_run: false,
            preconditions: decode_session_identity_preconditions_v1(preconditions)?,
        })),
        "item.check" => {
        ...
        "item.record_many" => {
            require_exact_keys_v1(payload, &["operations"])?;
            let operations =
                decode_item_record_many_operations_v1(value_v1(payload, "operations")?.clone())
                    .map_err(|_| invalid_execution_v1("item record-many operations are invalid"))?;
            Ok(SliceCommandV1::ItemRecordMany(
                podway_protocol::ItemRecordManyV1 {
                    operations,
                    preconditions: decode_session_preconditions_v1(preconditions)?,
                },
            ))
...
mod tests {
    ...
    #[test]
    fn v2agt004_batch_execution_document_round_trips_canonical_operations() {
        let admitted = AdmittedProcedureV2MutationV1 {
            selector: WorktreeSelectorWireV1::new(b"/tmp/worktree", "/tmp/worktree", None).unwrap(),
            workspace_id: WorkspaceId::new("00000000-0000-4000-8000-000000000924").unwrap(),
            command: SliceCommandV1::ItemRecordMany(podway_protocol::ItemRecordManyV1 {
                operations: vec![podway_protocol::ItemRecordManyOperationV1 {
                    item_id: ItemId::new("summary").unwrap(),
                    expected_item_revision: Revision::ZERO,
                    disposition: ItemRecordManyDispositionV1::Record {
                        record: ItemRecordValueV1::Text {
                            value: "done".to_owned(),
                        },
                    },
                }],
        ...
        assert!(decoded.attached_artifacts.is_empty());
    }
