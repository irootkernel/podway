...
fn registered_command_catalog_route_availability() -> BTreeMap<String, String> {
    ...
    assert_eq!(
        routes.len(),
        59,
        "the registered command catalog must contain the 53 executable routes and 6 reserved v2 routes"
    );
    routes
}
...
const ROUTE_SURFACES: &[RouteSurface] = &[
    ...
    RouteSurface {
        route: "procedure.convert",
        parser: &["procedure", "convert", "missing.yaml"],
        flags: &["--json", "--no-color", "--quiet"],
        values: &[],
        help_tokens: &["<file>"],
        dynamic: None,
    },
...
fn pac_048_recording_daemon_contract_table_validates_successful_versioned_json_output_for_every_route()
 {
    ...
    assert_eq!(
        ...
        "availability must identify exactly the current 53-route CLI surface",
    );
...
"#,
    ...
    let local_successes = [
        ...
        // Convert is the one authoring route whose input is a *v1* document, so it reuses the v1
        // fixture the validate and show rows read rather than the v2 one.
        (
            "procedure.convert",
            vec![
                "--json".to_owned(),
...
fn all_public_route_grammars_parse_to_a_single_structured_outcome() {
    let routes: &[(&str, &[&str])] = &[
        ...
        (
            "procedure.graph",
            &["procedure", "graph", "missing.yaml", "--format", "json"],
        ),
        ...
        (
            "procedure.convert",
            &["procedure", "convert", "missing.yaml"],
        ),
    ...
    for (route, arguments) in routes {
        ...
        match *route {
            ...
            | "procedure.graph" | "procedure.lint" | "procedure.check" | "procedure.convert" => {
...
fn public_route_surface_table_keeps_parser_help_and_completion_in_lockstep() {
    assert_eq!(ROUTE_SURFACES.len(), 53);
