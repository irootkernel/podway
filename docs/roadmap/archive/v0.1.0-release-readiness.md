...
Podway used one executable source-readiness gate: `make test` from the repository
root. That command was authoritative only for requirements and evidence registered
in the tested tree.
...
| Target | Scope |
|---|---|
| `make test-prepare` | canonical asset validation, formatting, lint, dependency policy, architecture guardrails, product-acceptance mapping, crash-boundary mapping, and contract checks |
