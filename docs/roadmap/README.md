...
| `V2LIF-006` | Version lifecycle-aware durable job wrappers | Planned | Preserve the released v3 job wrappers unchanged and emit new closed v4 wrappers for prepared-lifecycle commands. | [JSON contract](../specs/interfaces/json-contract.md) |
| `V2LIF-007` | Restore complete durable job read-back | Planned | Keep workspace initialization and lifecycle job receipts readable individually and as one ordered bounded job list. | [automation client contract](../specs/interfaces/automation-client-contract.md) |
