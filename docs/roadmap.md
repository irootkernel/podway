...
| `RECON003` | Preserve receipt-only lookup | Completed | Return the immutable terminal response projection after terminal job pruning. | [AUT-RECON-003](reference/interfaces/34-automation-client-contract.md#18-job-lookup-by-idempotency-key-aut-recon-001004) |
...
| `RECON005` | Test response-loss recovery | Completed | Cover response loss, wait timeout, every job state, pruning, and idempotent reconciliation. | [AUT-T-RECON](reference/interfaces/34-automation-client-contract.md#24-acceptance-matrix) |
