...
- Adding the literal `max_total_length` default rotates every newly canonicalized Procedure v2 digest exactly once. Shipped preset digests are re-pinned in the owning task, stored sessions keep their stored canonical snapshots, and digest-pinned automation receives `DIGEST_CONFIRMATION_REQUIRED` until it re-pins.
