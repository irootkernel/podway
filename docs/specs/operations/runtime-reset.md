...
`runtime.reset.plan`, confirmed single-mode and all-mode `runtime.reset.apply`, and
the internal `daemon.runtime_reset` control route. Planning inspects live daemons
and can issue a token for an idle, supported process. All-mode apply freezes and
retires the complete eligible ordinary-runtime selection recorded by that token.
