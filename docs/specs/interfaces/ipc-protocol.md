...
without registering six of the thirteen future command routes; `procedure.format`, `procedure.vet`, `procedure.graph`, `procedure.lint`, `procedure.check`, `procedure.scaffold`, and `procedure.convert` are registered and served. It validates required
