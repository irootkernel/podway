...
`podway.workspace-mode-switch-marker/v1`. The daemon admits these routes only
after config, registry, Store, source idleness, and a live Unix connection in the
exact target-mode namespace agree. This liveness probe does not create a
daemon-to-daemon protocol client or establish full target readiness.
Mode disagreement returns non-retryable `WORKSPACE_MODE_MISMATCH` before normal
Store access.
...
`V2SCL-002` reserved the route, the `podway.evidence-read-result/v1` family, and the error codes, and `V2SCL-004` serves them.
...
Text sizes and offsets count Unicode scalar values, list sizes and offsets count entries, and scalar item types use a single-page unit. Text pages end only at scalar boundaries and list pages contain complete entries, so no page splits an entry. Encoded page data is at most 256 KiB and the complete encoded response, including one token and its envelope, is at most 320 KiB. The daemon measures its result member against that bound and refuses to serve a response over it; the 64 KiB between the two bounds covers the envelope the transport adds, which a conformance test asserts on a maximal page.
...
Observation composes its own budget rather than embedding an unbounded duplicate of `session.next`. Its guidance emits evidence metadata only, with no previews or page tokens, and its evidence metadata window may be truncated. The active-item and mutation-template members are the two byte-budgeted windows. Compact-status items are a bounded collection inside the measured status member, not a window of their own: a status member that outgrows its allocation fails closed rather than cutting its item array. The active-item window admits in declaration order until its allocation is spent. The template window first reserves the first template of every command, so no action named by `allowed_actions` loses its template, and then fills the remainder, admitting a later template that fits after skipping one that does not. Active-item choice constraints expose at most eight choices plus the exact choice count and a truncation flag, while the complete declaration remains available from the Procedure inspection routes. Template argv elements are limited to 4,096 scalars.

Every derived collection was audited against the declaration it projects. A collection whose ceiling equals its declaration's own ceiling is complete and carries no truncation flag: decision options, evidence guidance, instructions, allowed actions, graph counters, and manual rework targets are all of this kind. A collection that can be narrower than what a declaration may produce is a window, and a window always publishes its exact total and whether it was cut. Raising items per definition to 128 left three collections in the standard status tier able to exceed their own published ceilings: `missing_required_item_ids`, `item_values`, and the `items_total` that describes it. `status-result/v3` is released and served, so each was corrected by raising its numeric ceiling to what a definition can declare rather than by windowing it. Windowing would have required a new member, and a released family never grows one; a bounded numeric update changes no field set and no unknown-field closure, which is the only in-place change the contract permits.
...
The version-aware result registry reserves the complete Procedure v2 family set. Every route in the v2 delta is registered, `availability` records which of them the current build serves, and a peer must reject a registered route it cannot serve with a structured compatibility error rather than ignoring it. `evidence.read` is registered and executable. The production decoder validates
