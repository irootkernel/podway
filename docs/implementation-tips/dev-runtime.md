...
`contributor` `podway.managed-runtime/v3` metadata, and drives only that
snapshot through named mode `contributor`.
...
- `daemon` builds with `--locked` and the daemon-only
  ...
  `exec`s the snapshot `podwayd --mode contributor` in the foreground so signal handling stays
  with the daemon process.
- `init` creates one managed disposable Git sandbox under the runtime root,
  runs the snapshotted `podway --mode contributor init` only against that sandbox, and only
...
- `run -- <podway args>` invokes the same snapshot CLI from the managed sandbox,
  prepends `--mode contributor`, and rejects `--socket`, `--worktree`, `--mode`,
  `--dev`, daemon
...
```text
/private/tmp/podway-contributor-<uid>-<12-char-sha256-of-canonical-checkout>
```
...
| Path | Role |
|---|---|
| `account/` | private account root used by debug test-isolation safeguards |
| `run/`, `state/`, `logs/` | mode-keyed runtime lock, socket, registry, recovery, service state, and logs |
...
the freshly derived checkout-derived root, effective uid, mode, canonical paths,
sandbox, and purpose identity. A running managed daemon keeps serving its adopted snapshot until you
stop it and start `daemon` again.

## Production coexistence

The managed runtime declares its canonical root through `PODWAY_DEV_HOME` and
keeps a private account root for debug test-isolation safeguards. The daemon
validates the v3 topology before deriving paths. That makes the singleton lock disjoint from the real
account's `~/.podway/run/podwayd.lock`, so the managed daemon can coexist with an
installed production LaunchAgent.

Raw `podwayd --mode contributor` without this helper lacks valid managed metadata
and fails closed. The compatibility alias `podwayd --dev` selects mode `dev`, not
the contributor runtime. Packaged qualification uses the same v3 metadata schema
with purpose `release-qualification` and mode `release-qa` under a unique temporary
managed root; it does not enable development-v2 admission.
...
debug-only feature, `contributor` launch mode, managed runtime metadata, snapshotted
...
place of public admission and therefore refuses raw contributor daemons, installed
