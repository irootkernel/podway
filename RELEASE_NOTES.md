...
- Preserve explicit `reset --all` recovery when workspace binding identity is
  readable but disposable full-store openability or internal-codec inspection
  fails.
...
- Serialize concurrent daemon log writers and refresh active-file identity and
  length before each write so singleton startup failures cannot exceed the
  retained 1-MiB per-file bound.
